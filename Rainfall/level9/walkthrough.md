...
What else do we have: by address `0x0804a008` lies function and there are 3 jumps to method `_ZN1NplERS_` from its address. Than starts the buffer. We can start our exploit from 4th byte of the buffer from address `0x804a010` that is \x10\xa0\x04\x08 in little-endian (because in `_ZN1N13setAnnotationEPc` there is jum